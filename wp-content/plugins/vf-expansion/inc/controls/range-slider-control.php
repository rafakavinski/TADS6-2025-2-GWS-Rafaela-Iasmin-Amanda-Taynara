 ...
 * @access public
 */
class Vf_Expansion_slider_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script(
			...
			VF_EXPANSION_PLUGIN_URL . '/inc/controls/assets/js/controls.js',
			array(
				'jquery',
				'customize-base',
				'jquery-ui-button',
		...
		wp_enqueue_style( 'vf_expansion-controls', VF_EXPANSION_PLUGIN_URL . '/inc/controls/assets/css/controls.css' );
	}
	...
	/**
	 * Vf_Expansion_slider_Control constructor.
	 *
 ...
 */
function vf_expansion_sanitize_range_value( $input ) {
	if ( vf_expansion_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function vf_expansion_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
